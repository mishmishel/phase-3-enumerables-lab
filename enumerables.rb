...
def get_names(spicy_foods)
  ...
  spicy_foods.each do |food|
    # Access the 'name' key in the hash and add it to the names array
    names << food[:name]
  end
...
def spiciest_foods(spicy_foods)
  ...
  # Use the filter method to select foods with heat_level > 5
...
def print_spiciest_foods(spicy_foods)
  ...
  # Use the filter method to select foods with heat_level > 5
  spiciest = spicy_foods.filter { |food| food[:heat_level] > 5 }.each do |food|
    puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{"🌶" * food[:heat_level]}"
  end
...
def average_heat_level(spicy_foods)
  ...
  average.round  # Round the result to the nearest integer
end
